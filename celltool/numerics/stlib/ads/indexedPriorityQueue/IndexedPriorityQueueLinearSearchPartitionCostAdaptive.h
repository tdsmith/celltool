...
/*!
  \file ads/indexedPriorityQueue/IndexedPriorityQueueLinearSearchPartitionCostAdaptive.h
  \brief Indexed priority queue that uses linear search.
*/

#if !defined(__ads_indexedPriorityQueue_IndexedPriorityQueueLinearSearchPartitionCostAdaptive_h__)
#define __ads_indexedPriorityQueue_IndexedPriorityQueueLinearSearchPartitionCostAdaptive_h__
...
class IndexedPriorityQueueLinearSearchPartitionCostAdaptive :
  public _Base {
  //
  // Enumerations.
  //
...
private:
  ...
  typename ads::Array<1, Iterator>::iterator _partitionEnd;
  Key _splittingValue;
...
public:
  ...
  IndexedPriorityQueueLinearSearchPartitionCostAdaptive(const int size) :
    Base(size),
...
private:
  ...
  void
  partition() {
    const Key increase = 1.1;
    const Key decrease = 0.9;

    // Determine a new splitting offset to try to balance the costs.
    if (_searchCount * _partitionSize < _costConstant * _keys.size()) {
      _splittingOffset *= increase;
    }
    else {
      _splittingOffset *= decrease;
    }
  ...
  void
  partitionInitial() {
    const int targetPartitionSize = std::max(1, int(std::sqrt(_keys.size())));
    // Partition the elements in the queue.
    std::nth_element(getQueueBeginning(), 
		     getQueueBeginning() + targetPartitionSize - 1,
		     getQueueEnd(), _compare);

    _partitionEnd = getQueueBeginning() + targetPartitionSize;
