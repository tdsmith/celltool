...
import celltool.numerics.procustes as procustes
import celltool.utility.path as path
import celltool.utility.numpy_compat as numpy_compat
...
#      self.points[-1] = self.points[0]
  ...
  def to_spline(self, smoothing = 0, spacing_corrected = False):
    ...
    the spline. This 'tck' tuple can be used by the routines in celltool.numerics.fitpack,
    or scipy.fitpack if scipy is installed. 'u' is a list of the parameter values
    corresponding to the points in the range.
    """
    import celltool.numerics.fitpack as fitpack
  ...
  def to_bezier(self, match_curves_to_points = False, smooth = True):
    ...
    if match_curves_to_points:
      to_insert = numpy.setdiff1d(u, numpy_compat.unique(tck[0]))
      for i in to_insert:
  ...
  def resample(self, num_points, smoothing = 0, max_iters = 500, min_rms_change = 1e-6, step_size = 0.2):
    ...
    from celltool.numerics.fitpack import splev
...
class PCAContour(Contour):
  ...
  def from_contours(cls, contours, required_variance_explained = 0.95, return_positions = False):
    ...
    """
    import celltool.numerics.pca as pca
...
class CentralAxisContour(Contour):
  ...
  # def resample_axis(self, num_points, smoothing = 0, max_iters = 500, min_rms_change = 1e-6, step_size = 0.2):
  #   # cache functions in inner loop as local vars for faster lookup
  #   from celltool.numerics.fitpack import splev
  #   norm, roll, clip, mean = utility_tools.norm, numpy.roll, numpy.clip, numpy.mean
  #   iters = 0
  #   ms_change = numpy.inf
  #   l = len(self.central_axis)
  #   tck, u = self.axis_to_spline(smoothing, spacing_corrected = True, end_weight = num_points / 2)
  #   positions = numpy.linspace(0, l, num_points, endpoint = True)
  #   min_ms_change = min_rms_change**2
  #   axis = numpy.transpose(splev(positions, tck))
  #   while (iters < max_iters and ms_change > min_ms_change):
  #     forward_distances = norm(axis - roll(axis, -1, axis = 0))
  #     backward_distances = norm(axis - roll(axis, 1, axis = 0))
  #     arc_spans = (roll(positions, -1, axis = 0) - roll(positions, 1, axis = 0)) % l
  #     deltas = forward_distances - backward_distances
  #     units = arc_spans / (forward_distances + backward_distances)
  #     steps = step_size * deltas * units
  #     steps[0] = steps[-1] = 0
  #     positions += steps
  #     positions = clip(positions, 0, l)
  #     iters += 1
  #     ms_change = mean((steps**2))
  #     axis = numpy.transpose(splev(positions, tck))
  #   self.central_axis = axis
  #   return iters, numpy.sqrt(ms_change)
  ...
  ### old version below
  # _instance_data.update({'axis':numpy.zeros((0, 2)), 'radii':numpy.zeros((0,)),
  #     'start':0, 'end':0})
  #
  # def from_contour(cls, contour, start, end, num_points, optimize_axis=True):
  #   import celltool.numerics.fitpack as fitpack
  #   tck, uout = contour.to_spline()
  #   start_pos, end_pos = numpy.transpose(fitpack.splev([start, end], tck))
  #   min_rms_change = numpy.sqrt(((start_pos - end_pos)**2).sum()) * 1e-4
  #   # strip away non-base-class contour attributes
  #   basic_contour = Contour(other=contour)
  #   contour = cls(other=basic_contour, axis=numpy.array([start_pos, end_pos]),
  #     radii=numpy.array([0.0, 0.0]), start=start, end=end)
  #   for i in range(4):
  #     contour._subdivide_axis()
  #     if i >= 2 and optimize_axis:
  #       contour.center_and_space_axis(centering_step=0.1, spacing_step=0.1,
  #         endpoint_step=1, min_rms_change=min_rms_change)
  #   if num_points != 17:
  #     tck, uout = contour.axis_to_spline(spacing_corrected=True)
  #     contour.axis = numpy.transpose(fitpack.splev(numpy.linspace(0, uout[-1], num_points, endpoint=True), tck))
  #   if optimize_axis:
  #     contour.center_and_space_axis(centering_step=0.1, spacing_step=0.1,
  #       endpoint_step=1, min_rms_change=min_rms_change)
  #   else:
  #     contour.recalculate_radii()
  #   return contour
  # from_contour = classmethod(from_contour)
  #
  # def update_axis_endpoints(start, end, tck=None):
  #   """Given a new start and endpoint (in terms of the contour parameter),
  #   update the physical position of the axes."""
  #   import celltool.numerics.fitpack as fitpack
  #   if tck is None:
  #     tck, uout = contour.to_spline()
  #   self.axis[[0, -1]] = numpy.transpose(fitpack.splev([start, end], tck))
  #   self.start = start
  #   self.end = end
  #
  # def _subdivide_axis(self):
  #   d1 = (self.axis[1:] - self.axis[:-1])
  #   midpoints = self.axis[:-1] + d1 / 2.0
  #   normals = numpy.empty((len(d1), 2))
  #   normals[:,0] = d1[:,1]
  #   normals[:,1] = -d1[:,0]
  #   intersect_points, arc_pos = self.find_shape_intersections(midpoints, midpoints+normals)
  #   mid_intersection = intersect_points.sum(axis=1) / 2.0
  #   new_midpoints = midpoints + mid_intersection[:, numpy.newaxis] * normals
  #   new_axis = numpy.empty((len(self.axis)+len(new_midpoints), 2), dtype=float)
  #   new_axis[::2] = self.axis
  #   new_axis[1::2] = new_midpoints
  #   self.axis = new_axis
  #   new_radii = numpy.empty(len(self.axis), dtype=float)
  #   new_radii[::2] = self.radii
  #   radii_fractions = numpy.absolute(intersect_points[:,0] - intersect_points[:,1]) / 2
  #   new_radii[1::2] = radii_fractions * numpy.sqrt((normals**2).sum(axis = 1))
  #   self.radii = new_radii
  #
  # def recalculate_radii(self):
  #   normals = self.axis_normals(False)[1:-1]
  #   intersect_points, arc_pos = self.find_shape_intersections(self.axis[1:-1], self.axis[1:-1]+normals)
  #   self.radii = numpy.empty(len(self.axis), dtype=float)
  #   self.radii[1:-1] = numpy.absolute(intersect_points[:,0] - intersect_points[:,1]) / 2
  #   self.radii[[0, -1]] = 0
  #
  # def center_and_space_axis(self, max_iters=500, min_rms_change=1e-6,
  #       centering_step=0.1, spacing_step=0.1, endpoint_step=1, straight_step = 0,
  #       endpoint_spring=0, store = False):
  #   # cache functions in inner loop as local vars for faster lookup
  #   from celltool.numerics.fitpack import splev
  #   find_shape_intersections = self.find_shape_intersections
  ...
  #   tck, uout = self.to_spline()
  #   min_ms_change = min_rms_change**2
  #   inner_axis = self.axis[1:-1]
  #   axis, start, end = self.axis, self.start, self.end
  #   deviation = numpy.empty((l, 2), dtype=float)
  #   if store:
  #     positions = [numpy.array(axis, copy=True)]
  #   while (iters < max_iters and ms_change > min_ms_change):
  #     offsets = axis[:-1] - axis[1:]
  #     prev_minus_current = offsets[:-1]
  #     next_minus_current = -offsets[1:]
  #     next_minus_start = -offsets[0]
  #     prev_minus_end = offsets[-1]
  #     kink_directions = prev_minus_current + next_minus_current
  #     spacing_dir = spacing_step * kink_directions
  #
  #     import celltool.numerics.ndimage as nd
  #     spacing_dir = nd.gaussian_filter1d(spacing_dir, 4, axis=0)
  #     straightening_dir = straight_step * (kink_directions[:-2] - 2*kink_directions[1:-1] + kink_directions[2:])
  #     straightening_dir = nd.gaussian_filter1d(straightening_dir, 4, axis=0)
  #     d1 = (axis[2:] - axis[:-2])/2.0
  #     normals = numpy.empty((l2, 2))
  #     normals[:,0] = d1[:,1]
  #     normals[:,1] = -d1[:,0]
  #     intersections, arc_pos = find_shape_intersections(inner_axis, inner_axis+normals)
  #     radius_1, radius_2 = intersections.T
  #     centering_dir = centering_step * ((radius_1 + radius_2) / 2)[:, newaxis] * normals
  #     centering_dir = nd.gaussian_filter1d(centering_dir, 4, axis=0)
  #     inner_axis_step = spacing_dir + centering_dir
  #     inner_axis_step[1:-1] += straightening_dir
  #     inner_axis += inner_axis_step
  #     startd, endd = numpy.transpose(splev([start, end], tck, 1))
  #     start_dir = -endpoint_step * dot(prev_minus_current[0] + next_minus_current[0], startd / (startd**2).sum())
  #     end_dir = -endpoint_step * dot(prev_minus_current[-1] + next_minus_current[-1], endd / (endd**2).sum())
  #     start_dir += -endpoint_spring * dot(prev_minus_current[0], startd / (startd**2).sum())
  #     end_dir += -endpoint_spring * dot(next_minus_current[-1], endd / (endd**2).sum())
  #     start += start_dir
  #     end += end_dir
  #     start %= num_points
  #     end %= num_points
  #     new_start, new_end = transpose(splev([start, end], tck))
  #     deviation[0] = axis[0] - new_start
  #     deviation[1:-1] = inner_axis_step
  #     deviation[-1] = axis[-1] - new_end
  #     axis[0] = new_start
  #     axis[-1] = new_end
  #     ms_change = (deviation**2).sum(axis=1).mean()
  #     iters += 1
  #     if store:
  #       positions.append(numpy.array(axis, copy=True))
  #   self.start, self.end = start, end
  #   self.recalculate_radii()
  #   if store:
  #     return iters, numpy.sqrt(ms_change), positions
  #   return iters, numpy.sqrt(ms_change)
  #
  # def transform(self, transform):
  #   Contour.transform(self, transform)
  #   self.central_axis = utility_tools.homogenous_transform_points(self.axis, transform)
  #
  # def axis_cumulative_distances(self):
  #   return numpy.add.accumulate(self.axis_interpoint_distances())
  #
  # def axis_length(self):
  #   return self.axis_interpoint_distances().sum()
  #
  # def axis_interpoint_distances(self):
  #   distances = utility_tools.norm(self.axis - numpy.roll(self.axis, 1, axis = 0), axis = 0)
  #   distances[0] = 0
  #   return distances
  #
  # def axis_to_spline(self, smoothing = 0, spacing_corrected = False, end_weight = None):
  ...
  #   else:
  #     weights = None
  #   tck, uout = fitpack.splprep(x = self.axis.transpose(), u = u, s = smoothing, w = weights)
  #   return tck, uout
  #
  # def axis_to_bezier(self, match_curves_to_points = False):
  #   import celltool.numerics.fitpack as fitpack
  ...
  #   return iters, numpy.sqrt(ms_change)
  #
  # def axis_normals(self, spline_derivative = True):
  #   num_points = len(self.axis)
  #   if spline_derivative:
  #     import celltool.numerics.fitpack as fitpack
  #     tck, u = self.axis_to_spline()
  #     d1 = numpy.transpose(fitpack.splev(numpy.arange(num_points), tck, der=1))
  #   else:
  #     d1 = utility_tools.first_derivative(self.axis)
  #   normals = numpy.empty((num_points, 2))
  #   normals[:,0] = d1[:,1]
  #   normals[:,1] = -d1[:,0]
  #   normals /= numpy.sqrt((normals**2).sum(axis = 1))[:,numpy.newaxis]
  #   return normals
  #
  # def make_points_from_axis(self):
  #   normals = self.axis_normals()
  #   outline = [point+normal*radius for point, normal, radius in zip(self.axis, normals, self.radii)]
  #   outline += [point-normal*radius for point, normal, radius in reversed(zip(self.axis, normals, self.radii))][1:-1]
  #   old_points = self.points
  #   self.points = numpy.array(outline)
  #   return old_points
  #
  # def straighten(self):
  #   new_axis = numpy.zeros_like(self.axis)
  #   new_axis[:,0] = self.axis_cumulative_distances()
  #   self.axis = new_axis
  #   self.make_points_from_axis()
  #
  # def _to_gnuplot(self, normals = None):
  #   import Gnuplot
  #   if normals is None:
  #     normals = self.axis_normals()
  #   ribs = [Gnuplot.Data([c+norm*rad, c-norm*rad], with_='lines lt 1') for c, norm, rad in zip(self.axis, normals, self.radii)]
  #   axis = Gnuplot.Data(self.axis, with_='lines lt 2')
  #   outline = Gnuplot.Data(self.points, with_='lines lt 3')
  #   return ribs + [axis, outline]
  #
  # as_axis_centered_and_spaced = _copymethod(center_and_space_axis)
  # as_resampled_axis = _copymethod(resample_axis)
  # as_points_from_axis = _copymethod(make_points_from_axis)
  # as_straightened = _copymethod(straighten)
